...
int getWeightByBlueStack() {
    const char *bluestacks_path[22] = {
            ...
            "/data/app/com.bluestacks.searchapp-1.apk", "/data/bluestacks.prop",
...
int getWeightByTelephoneNum(JNIEnv *env, jobject mContext) {
    ...
    const char *simulatorTelephoneNum[16] = {
...
int getWeightByHardware(JNIEnv *env) {
    ...
    if (strcmp(jstringToChar(env, ret_str), "goldfish") ||
        strcmp(jstringToChar(env, ret_str), "ranchu")) {
        return 2;
    } else {
        return 0;
    }
...
int getWeightByBrand(JNIEnv *env) {
    ...
    if (strstr(jstringToChar(env, ret_str), "generic") ||
        strstr(jstringToChar(env, ret_str), "Android")) {
        return 2;
    } else {
        return 0;
    }
...
int simulatorDetected(JNIEnv *env, jobject mContext, const int threshold) {
    ...
    if (threshold > weight_total && weight_total > 10) {
        return 0;
    }
    return -1;
