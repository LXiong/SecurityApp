...
jobject getTelephonyManager(JNIEnv *env, jobject mContext) {
    ...
    jmethodID METHOD_getSystemService = env->GetMethodID(system_context, "getSystemService",
    ...
    jfieldID FIELD_TELEPHONY_SERVICE = env->GetStaticFieldID(system_context, "TELEPHONY_SERVICE",
                                                             "Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionDescribe();
        env->ExceptionClear();
    ...
    jstring Value_TELEPHONY_SERVICE = (jstring) env->GetStaticObjectField(system_context,
                                                                          FIELD_TELEPHONY_SERVICE);
    if (env->ExceptionCheck()) {
        env->ExceptionDescribe();
        env->ExceptionClear();
        return NULL;
    ...
    jobject Object_telephone_manager = env->CallObjectMethod(mContext, METHOD_getSystemService,
                                                             Value_TELEPHONY_SERVICE);
    if (env->ExceptionCheck()) {
        env->ExceptionDescribe();
        env->ExceptionClear();
        return NULL;
...
jstring getIMEI(JNIEnv *env, jobject mContext) {
    ...
    if (OBJECT_telephone_manager == NULL) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "OBJECT_telephone_manager get failed");
    }
    jclass CLASS_TelephonyManager = env->FindClass("android/telephony/TelephonyManager");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "CLASS_Telephonymanager get failed Excption");
    }
    if (CLASS_TelephonyManager == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "CLASS_Telephonymanager get failed");
    }
    jmethodID METHOD_getDeviceId = env->GetMethodID(CLASS_TelephonyManager, "getDeviceId",
                                                    "()Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getDeviceId get failed Excption");
    }
...
jstring getIMSI(JNIEnv *env, jobject mContext) {
    ...
    jmethodID METHOD_getSubscriberId = env->GetMethodID(CLASS_TelephonyManager, "getSubscriberId",
                                                        "()Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getSubscriberId get failed Excption");
    }
    if (METHOD_getSubscriberId == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getSubscriberId get failed");
    }

    jstring retStr = (jstring) env->CallObjectMethod(OBJECT_telephone_manager,
                                                     METHOD_getSubscriberId);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "IMEI Value get failed Excption");
    }
    return retStr;
}
...
jstring getTelephoneNum(JNIEnv *env, jobject mContext) {
    ...
    jmethodID METHOD_getLine1Number = env->GetMethodID(CLASS_TelephonyManager, "getLine1Number",
                                                       "()Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getLine1Number get failed Excption");
    }
    if (METHOD_getLine1Number == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getLine1Number get failed");
    }
    jstring retStr = (jstring) env->CallObjectMethod(OBJECT_telephone_manager,
                                                     METHOD_getLine1Number);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "LineNumber value get failed Excption");
    }
    return retStr;
...
jstring getBuildBasicInfo(JNIEnv *env, const char *value) {
    ...
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "android/os/Build get failed Excption");
    }
    if (CLASS_ANDROID_OS_BUILD == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__, "android/os/Build get failed");
    }

    jfieldID FIELD_VALUE = env->GetStaticFieldID(CLASS_ANDROID_OS_BUILD, value,
                                                 "Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "FIELD_VALUE get failed Excption");
    }
    if (FIELD_VALUE == 0) {
        char msg[100] = "\0";
...
jstring getWlan0Mac(JNIEnv *env, jobject mContext) {
    ...
    jfieldID FIELD_WIFI_SERVICE = env->GetStaticFieldID(system_context, "WIFI_SERVICE",
                                                        "Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "FIELD_WIFI_SERVICE get failed Excption");
    }
    if (FIELD_WIFI_SERVICE == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__, "FIELD_WIFI_SERVICE get failed");
    }

    jstring VALUE_WIFI_SERVICE = (jstring) env->GetStaticObjectField(system_context,
                                                                     FIELD_WIFI_SERVICE);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "VALUE_WIFI_SERVER get failed Excption");
    }
    if (VALUE_WIFI_SERVICE == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "VALUE_WIFI_SERVICE get failed ");
    }
    ...
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getSystemService get failed Excption");
    }
    if (METHOD_getSystemService == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_getSystemService get failed");
    }
    jobject OBJECT_WIFI_MANAGER = env->CallObjectMethod(mContext, METHOD_getSystemService,
                                                        VALUE_WIFI_SERVICE);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "OBJECT_WIFI_MANAGER get failed Excption");
    }
    if (OBJECT_WIFI_MANAGER == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "OBJECT_WIFI_MANAGER get failed ");
    }
    jclass CLASS_WIFI_MANAGER = env->GetObjectClass(OBJECT_WIFI_MANAGER);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "CLASS_WIFI_MANAGER get failed Excption");
    }
    if (CLASS_WIFI_MANAGER == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__, "CLASS_WIFI_MANAGER get failed");
    }
    jmethodID METHOD_WIFI_INFO = env->GetMethodID(CLASS_WIFI_MANAGER, "getConnectionInfo",
                                                  "()Landroid/net/wifi/WifiInfo;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_WIFI_INFO get failed Excption");
    }
    ...
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "OBJECT_WIFI_INFO  get failed Excption");
    }
    ...
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "CLASS_WIFI_INFO get failed Excption");
    }
    if (CLASS_WIFI_INFO == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__, "CLASS_WIFI_INFO get failed ");
    }
    jmethodID METHOD_MAC_getMacAddress = env->GetMethodID(CLASS_WIFI_INFO, "getMacAddress",
                                                          "()Ljava/lang/String;");
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_MAC_getMacAddress get failed Excption");
    }
    if (METHOD_MAC_getMacAddress == 0) {
        return getErrorInfo(env, __FILE__, __FUNCTION__, __LINE__,
                            "METHOD_MAC_getMacAddress get failed ");
    }
